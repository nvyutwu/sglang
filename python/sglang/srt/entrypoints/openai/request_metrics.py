...
def classify_chat_request(request) -> None:
    ...
    _classify_structured_output(request)


def classify_completion_request(request) -> None:
    """Classify a CompletionRequest and increment counters.

    Completions support structured output (response_format, regex, ebnf)
...
def classify_responses_request(request) -> None:
    ...
    Responses support tools and images in input,
    but not response_format/regex/ebnf in SGLang.
    """
    # Images in input
...
def _classify_structured_output(request) -> None:
    """Check for structured output across any request type."""
    response_format = getattr(request, "response_format", None)
    if (
        response_format is not None
        and hasattr(response_format, "type")
        and response_format.type in ("json_schema", "json_object", "structural_tag")
    ):
        request_type_structured_output.inc()
        return
    if getattr(request, "regex", None) is not None:
        request_type_structured_output.inc()
        return
    if getattr(request, "ebnf", None) is not None:
        request_type_structured_output.inc()
        return
